...
import com.mcproject.net.util.ShareUtil;
...
public class PlayerActivity extends YouTubeFailureRecoveryActivity{
	...
	private boolean IS_MP3_DOWNLOAD = false;			// 음원추출중인지 여부	
...
//	}
	...
	private void download_btnClick(){
		AlertDialog.Builder alert = new Builder(PlayerActivity.this);
		
		alert.setTitle(R.string.mv_dn_dialog_title);
		alert.setMessage(R.string.mv_dn_dialog_msg);
...
//				set_favorite.setClickable(false);
				...
				else LogUtil.D("699 loadOtherVideo 1 : " + dto.title);
...
//				set_favorite.setClickable(false);
				if(dto.title != null) curr_title = dto.title;
				else LogUtil.D("707 loadOtherVideo 2 : " + dto.title);
				curr_mvId = dto.videoid;
				mPlayer.loadVideo(dto.videoid);
			}
